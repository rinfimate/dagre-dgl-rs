...
/// Restores edges that were reversed by [`run`] back to their original direction.
pub fn undo(graph: &mut Graph) {
    let reversed_edges: Vec<Edge> = graph
        .edges()
        .into_iter()
...
/// Computes a feedback arc set using a greedy bucket-sort approach.
pub fn greedy_fas(graph: &Graph) -> Vec<Edge> {
    ...
    results
        .into_iter()
        .flat_map(|(v, w)| graph.out_edges_to(&v, &w).unwrap_or_default())
        .collect()
}
...
fn build_fas_state(graph: &Graph) -> (Graph, Vec<Vec<String>>, usize) {
    ...
    for v in graph.nodes() {
        let n = crate::graph::NodeLabel {
            x: Some(0.0), // out_weight
            y: Some(0.0), // in_weight
            ..Default::default()
        };
    ...
    for edge in graph.edges() {
        ...
        let existing = fas_graph
            .edge_vw(&edge.v, &edge.w)
            .and_then(|l| l.weight)
            .unwrap_or(0.0);
        fas_graph.set_edge(
            &edge.v,
